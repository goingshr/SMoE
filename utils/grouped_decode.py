"""Graph replay of cached BF16 experts, preserving FP32 Xverse router scores.
...
class GroupedDecode:
    def __init__(self, hidden, intermediate, top_k, device, score_dtype=torch.float32):
        self.h, self.i, self.k = hidden, intermediate, top_k
        self.x = torch.zeros((1,hidden), dtype=torch.bfloat16, device=device)
        self.weights = torch.zeros((top_k,), dtype=score_dtype, device=device)
