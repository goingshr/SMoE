...
def _rope(Q,K,Cos,Sin,Pos,OQ,OK,D:tl.constexpr,NQ:tl.constexpr,NK:tl.constexpr,
          BLOCK:tl.constexpr, POSITIONED:tl.constexpr):
    r=tl.arange(0,BLOCK)
    col=r%D
    partner=r-col+(col+D//2)%D
    sign=tl.where(col<D//2,-1.,1.)
    position=tl.load(Pos) if POSITIONED else 0
...
def rope(q,k,cos,sin,position):
    ...
    _rope[(1,)](q,k,cos,sin,position,oq,ok,q.shape[-1],q.numel(),k.numel(),
        triton.next_power_of_2(max(q.numel(),k.numel())),position is not None,
...
def install(model):
    ...
    def forward(q,k,cos,sin,position_ids=None,unsqueeze_dim=1):
        tensors=(q,k,cos,sin)
        modern = (cos.ndim == sin.ndim == 3 and cos.shape[:2] == (1,1))
        legacy = (cos.ndim == sin.ndim == 2 and position_ids is not None
        ...
        if (enabled and not torch.is_grad_enabled() and unsqueeze_dim==1
                ...
                and q.shape[-1]%2==0 and (modern or legacy)
            ...
            return rope(q,k,cos,sin,None if modern else position_ids)
